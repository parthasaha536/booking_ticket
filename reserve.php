...
<h1 class="head"><b>Now Showing Movies</b></h1>
				...
				if($row['status']==1)
				{
					...
					else
					{
						if($t>$y)//$t=1 $y=0
						{
						  ...
						  echo"<input class='name' type='text' value='$row[name]' disabled>";
					      $j++;
...
<h1 class="head1"><b>Upcoming Movies . . .</b></h1><br><br>
...
<?php
		...
		$qury="select COUNT(status) as status from movie where status=2";
		$qury1=mysqli_query($connection,$qury);
		while($row=mysqli_fetch_array($qury1))
		{
			$p=$row['status'];
...
?>
			...
			<?php
				...
				//echo "<input type='text' value='$row[movie_id]' onClick='reply_click(this.id)";
