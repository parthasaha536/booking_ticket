...
<h1 class="head"><b>Make New Category</b></h1><br>

<span class="span1"><div class="form-group row">
<label for="c_name" class="col-md-5 col-form-label text-md-right"><b><h4>Category Name</h4></b></label>
<div class="col-md-7">
<input type="text"    placeholder="Enter Hare ..." class="form-control" name="c_name"   required>
</div>
</div></span>
...
if(isset($_POST['submit']))
{
	...
	$stop='1';
	$qq=mysqli_query($connection,"select category_id from category");
	while($row=mysqli_fetch_array($qq))
	{
		
	      if($stop==$row['category_id'])
		  {
			  $stop++;
	      }
	}
	if($stop==1)
	{
		$add="insert into category(name,category_id) values('$name','$stop')";
	    $re=mysqli_query($connection,$add);
	}
	$first=$stop-1;
	$qq1=mysqli_query($connection,"select * from category where category_id='$first' ");
	while($row=mysqli_fetch_array($qq1))
	{
		
		if($name==$row['name'])
		{
			$s='Category Name Already Registered !!';
